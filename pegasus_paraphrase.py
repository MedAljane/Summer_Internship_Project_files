...
torch_device = 'cuda' if torch.cuda.is_available() and torch.cuda.get_device_properties(
    0).total_memory >= 6442450944 else 'cpu'
tokenizer = PegasusTokenizer.from_pretrained(model_name)
model = PegasusForConditionalGeneration.from_pretrained(model_name).to(torch_device)


def get_response(input_text, num_return_sequences, num_beams) -> list:
    """
    Function for the generation of a paraphrased sentence
    :param input_text: String
    :param num_return_sequences: int
    :param num_beams: int
    :return: String
    """
